...
int NaiveBayesClassifier::Predict(const dataVec& x) {
...
double NaiveBayesClassifier::CalculateClassProbability(unsigned int classID,const dataVec& x){
...
void NaiveBayesClassifier::Train(const Dataset& dataset) {
    ...
    trainBayes(dataset,classProbs);
    delete[] classProbs;
}
void NaiveBayesClassifier::trainBayes(const Dataset& dataset,const float* classProbs){
